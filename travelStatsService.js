const { GpsLog, TravelStat } = require('./mongo');
const KM_TO_MILES = 0.621371;
const DEFAULT_INTERVAL_MIN = 20; // نفس الجدولة الحالية (يمكن رفعها لاحقاً)
const STOP_COUNT_THRESHOLD_MIN = 3; // احتساب مرة توقف إذا استمر >= 3 دقائق
const { calcDistanceDiffSafe } = require('./gpsJumpGuard');

/**
 * يبني إحصائيات الرحلات لليوم المحدد (UTC) بحسب حد التوقف stopThresholdMin (بالدقائق)،
 * ويخزّنها في TravelStat (upsert per imei/day/threshold).
 */
async function buildAndPersistTravelStats({ dayUtc, stopThresholdMin = 1, imeis = [] }) {
  const start = dayUtc ? new Date(dayUtc) : startOfTodayUTC();
  const end = new Date(start);
  end.setUTCDate(end.getUTCDate() + 1);

  const imeiList = imeis.length
    ? imeis
    : await GpsLog.distinct('imei', {
        date: { $gte: start, $lt: end },
        type: { $in: ['gps', 'alarm'] },
      });

  const results = [];

  for (const imei of imeiList) {
    const points = await GpsLog.find({
      imei,
      date: { $gte: start, $lt: end },
      type: { $in: ['gps', 'alarm'] },
    }).sort({ date: 1 }).lean();

    const { segments, totalKm, totalDriveMin, stopCount } = computeSegments(points, stopThresholdMin);

    const doc = {
      imei,
      day: start,
      stop_threshold_min: stopThresholdMin,
      segments,
      total_distance_km: totalKm,
      total_driving_minutes: totalDriveMin,
      total_segments: segments.length,
      total_stop_count: stopCount || 0,
    };

    await TravelStat.findOneAndUpdate(
      { imei, day: start, stop_threshold_min: stopThresholdMin },
      { $set: doc },
      { upsert: true }
    );

    results.push(doc);
  }

  return results;
}

/**
 * يحسب القطاعات (رحلات) حسب حد التوقف.
 */
function computeSegments(points, stopThresholdMin) {
  const segments = [];
  let seg = null;
  let totalKm = 0;
  let totalDriveMin = 0;
  let zeroStart = null;
  let zeroCounted = false;
  let stopCount = 0;
  let last = null;

  for (const p of points) {
    const t = new Date(p.date);
    const speed = Number(p.speed) || 0;
    const lat = p.latitude ?? p.gps?.latitude;
    const lon = p.longitude ?? p.gps?.longitude;

    // ⚠️ Driving mileage: نحسب المسافة بين النقطتين بشكل آمن لتفادي القفزات
    // (لا نعتمد على distanceDiff المخزن لأنه قد يحتوي قفزات تاريخية).
    if (seg && last) {
      const res = calcDistanceDiffSafe(
        { lat: last.latitude ?? last.gps?.latitude, lon: last.longitude ?? last.gps?.longitude, date: last.date },
        { lat, lon, date: p.date }
      );
      const prevSpeed = Number(last.speed) || 0;
      if (!res.isJump && (prevSpeed > 0 || speed > 0)) {
        seg.distance_km += res.distanceKm;
      } else if (res.isJump) {
        // تعليق مهم: عند اكتشاف قفزة، لا نضيف مسافة ولا نربط الرحلة بنقطة بعيدة
        // ويمكن لاحقاً اعتبارها Boundary للرحلة حسب احتياجك.
      }
    }

    // تقدير زمن القيادة بين النقطتين
    if (seg && last) {
      const deltaMin = Math.max(0, (t - new Date(last.date)) / 60000);
      const prevSpeed = Number(last.speed) || 0;
      if (prevSpeed > 0 || speed > 0) {
        seg.driving_minutes += deltaMin;
      }
    }

    if (speed > 0) {
      // أغلق توقف سابق إن وجد
      if (zeroStart && !zeroCounted) {
        const stopDur = (t - zeroStart) / 60000;
        if (stopDur >= STOP_COUNT_THRESHOLD_MIN) stopCount += 1;
      }
      zeroStart = null;
      zeroCounted = false;

      // بدء قطاع جديد عند أول حركة
      if (!seg) {
        seg = {
          start_at: t,
          end_at: t,
          distance_km: 0,
          driving_minutes: 0,
          start_loc: { lat, lon },
          end_loc: { lat, lon },
        };
      }
      seg.end_at = t;
      seg.end_loc = { lat, lon };
      zeroStart = null; // نسيان التوقف
      zeroCounted = false;
    } else {
      // سرعة صفر: ابدأ حساب مدة التوقف
      if (!zeroStart) zeroStart = t;
      const stopDur = (t - zeroStart) / 60000;

      // عدّ مرة توقف عند بلوغ 3 دقائق
      if (!zeroCounted && stopDur >= STOP_COUNT_THRESHOLD_MIN) {
        stopCount += 1;
        zeroCounted = true;
      }

      // فصل الرحلات بناءً على عتبة التوقف المختارة
      if (stopDur >= stopThresholdMin && seg) {
        // أغلق القطاع عند آخر نقطة قبل التوقف
        segments.push(seg);
        totalKm += seg.distance_km;
        totalDriveMin += seg.driving_minutes;
        seg = null;
      }
    }

    last = p;
  }

  // أغلق قطاع متبقٍ
  if (seg) {
    segments.push(seg);
    totalKm += seg.distance_km;
    totalDriveMin += seg.driving_minutes;
  }

  // احتساب توقف في نهاية اليوم إن لم يُحسب
  if (zeroStart && !zeroCounted) {
    // لا نعرف النقطة التالية، نستخدم آخر وقت معروف (last)
    const endT = last ? new Date(last.date) : zeroStart;
    const stopDur = (endT - zeroStart) / 60000;
    if (stopDur >= STOP_COUNT_THRESHOLD_MIN) stopCount += 1;
  }

  return { segments, totalKm, totalDriveMin, stopCount };
}

function startOfTodayUTC() {
  const now = new Date();
  return new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
}

module.exports = {
  buildAndPersistTravelStats,
  startTravelStatsScheduler,
};

/**
 * جدولة بناء تقرير الرحلات دورياً (افتراضياً كل 20 دقيقة)
 * stopThresholdsMinutes: مصفوفة قيم حد التوقف لحساب أكثر من سيناريو (مثلاً [1,2,5])
 */
function startTravelStatsScheduler({
  stopThresholdsMinutes = [1,2,3,4,5,10,15,20,25,30,45,60,360,720],
  intervalMinutes = DEFAULT_INTERVAL_MIN,
} = {}) {
  // تشغيل مبدئي
  runAll(stopThresholdsMinutes);
  setInterval(() => runAll(stopThresholdsMinutes), intervalMinutes * 60 * 1000);

  async function runAll(thresholds) {
    for (const t of thresholds) {
      try {
        await buildAndPersistTravelStats({ stopThresholdMin: t });
      } catch (err) {
        console.error(`Travel stats job error (stop ${t} min):`, err.message);
      }
    }
  }
}

