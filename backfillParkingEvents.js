...
/**
 * Backfill parkingevents documents from historic gpslogs data.
 *
 * Usage examples:
 *   node backfillParkingEvents.js                // default last 5 days, all IMEIs found
 *   node backfillParkingEvents.js --days=3       // only last 3 days
 *   node backfillParkingEvents.js --imei=123456  // single device
 *   node backfillParkingEvents.js --imei=123,456 // multiple devices
 */

const mongoose = require('mongoose');
const { GpsLog, ParkingEvent } = require('./mongo');

const START_SPEED_KPH = 1;
const RELEASE_SPEED_KPH = 5;
const VALID_TYPES = ['gps', 'alarm'];
...
function parseOptions(args) {
  ...
  for (const arg of args) {
    if (arg.startsWith('--days=')) {
      ...
      if (list) {
        opts.imeis = list.split(',').map(s => s.trim()).filter(Boolean);
      }
    }
  }
...
async function resolveImeis(imeis, start, end) {
  ...
  const found = await GpsLog.distinct('imei', {
    type: { $in: VALID_TYPES },
...
async function rebuildForImei(imei, start, end) {
  ...
  if (logs.length === 0) {
    console.log(`IMEI ${imei}: no logs in range, skipping.`);
    return;
  }
...
function buildEventsFromLogs(logs) {
  ...
  for (const log of logs) {
    const ts = toDate(log.date);
    if (!ts) continue;

    const lat = pickCoord(log, 'latitude');
    const lon = pickCoord(log, 'longitude');
    if (!isFiniteCoord(lat, lon)) continue;

    const speed = pickSpeed(log);
...
function pickCoord(log, key) {
  const value = log[key] ?? (log.gps ? log.gps[key] : undefined);
  const num = Number(value);
  return Number.isFinite(num) ? num : null;
}

function pickSpeed(log) {
  const value = log.speed ?? (log.gps ? log.gps.speed : 0);
  const num = Number(value);
