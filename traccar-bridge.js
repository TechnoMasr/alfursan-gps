...
const { createTraccarReconnect, createTraccarSilenceWatchdog } = require("./lib/traccarReconnect");
const { createDeviceResolver, createDevicesListBackoff } = require("./lib/deviceResolve");
const { partitionPositions, groupByDeviceId } = require("./lib/ingressPartition");
const { createRealtimeIngress } = require("./lib/realtimeIngress");
const { createEventLoopLagMonitor } = require("./lib/eventLoopLag");

// ====== CONFIG ======
const TRACCAR_BASE = "http://95.216.21.242:8082";
const TRACCAR_WS = "ws://95.216.21.242:8082/api/socket";
const USERNAME = process.env.TRACCAR_USER || "abdelrahman.technomasr@gmail.com";
const PASSWORD = process.env.TRACCAR_PASS || "abdelrahman";
const BRIDGE_ENV = loadBridgeEnv();
const SUBSCRIBERS_SERVER_PORT = Number(process.env.TRACCAR_BRIDGE_PORT || 2053);
const DEVICES_POLL_MS = BRIDGE_ENV.DEVICES_POLL_MS;
const DEVICE_FETCH_COOLDOWN_MS = BRIDGE_ENV.DEVICE_FETCH_COOLDOWN_MS;
const DEVICES_LIST_BACKOFF_BASE_MS = BRIDGE_ENV.DEVICES_LIST_BACKOFF_BASE_MS;
const DEVICES_LIST_BACKOFF_MAX_MS = BRIDGE_ENV.DEVICES_LIST_BACKOFF_MAX_MS;
const WS_REFRESH_MS = BRIDGE_ENV.WS_REFRESH_MS;
...
const IGNORED_ALARM_CODES = new Set(["tampering"]);

/**
 * Legacy HTTPS :2053 / old WSS — subscribers relay only.
 * Persistence, FCM, and schedulers run in traccar-bridge-ontherport.js.
 */
const RELAY_ONLY_LEGACY = true;
// ====================

let mongooseMod = null;
function getMongoose() {
  if (mongooseMod) return mongooseMod;
  try {
    mongooseMod = require("mongoose");
    require("./mongo");
    return mongooseMod;
  } catch (err) {
    console.warn("[legacy-relay] Mongo not loaded (GPS relay continues):", err.message);
    return null;
  }
}

const sslOptions = {
  key: fs.readFileSync("./ssl/alfursantracking.com.key"),
  cert: fs.readFileSync("./ssl/alfursantracking.com.cert"),
};
...
/**
 * Global fast channel for command replies (all clients).
...
/** عدد اتصالات WSS النشطة على منفذ legacy (قبل/بعد subscribe). */
let legacyWssClientCount = 0;
let legacyWssClientSeq = 0;
...
let traccarCookieHeader = null;
let traccarWs = null;
let traccarIngressStarting = false;
let closingForReconnect = false;
let closingForSubscriberIdle = false;
...
function countDeviceSubscriberSockets() {
  let n = 0;
...
function getSubscriberMonitorSnapshot(extra = {}) {
  return {
    wss_clients: legacyWssClientCount,
    subscribed_imeis: deviceSubscribers.size,
    device_subscriptions: countDeviceSubscriberSockets(),
    traccar_ws_open: !!(traccarWs && traccarWs.readyState === WebSocket.OPEN),
    ...extra,
  };
}

/** مراقبة اتصال/فصل العملاء على منفذ legacy. */
function logSubscriberMonitor(event, extra = {}) {
  const line = getSubscriberMonitorSnapshot({ event, ...extra });
  console.log(`[legacy-ws] ${event} | متصلون الآن: ${line.wss_clients} | اشتراكات أجهزة: ${line.device_subscriptions} (${line.subscribed_imeis} imei) | Traccar WS: ${line.traccar_ws_open ? "open" : "off"}`);
}

function attachLegacyWssClient(ws) {
  legacyWssClientSeq += 1;
  ws._legacyClientId = `ws-${legacyWssClientSeq}`;
  ws._legacyDetachDone = false;
  legacyWssClientCount += 1;
  logSubscriberMonitor("عميل متصل", { client_id: ws._legacyClientId });
}

function detachLegacyWssClient(ws, reason = "close") {
  if (!ws || ws._legacyDetachDone) return;
  ws._legacyDetachDone = true;
  const clientId = ws._legacyClientId || "unknown";
  const imeisBefore = ws.subscriptions ? [...ws.subscriptions] : [];
  removeClientFromAll(ws);
  if (legacyWssClientCount > 0) legacyWssClientCount -= 1;
  logSubscriberMonitor("عميل منفصل", {
    client_id: clientId,
    reason,
    had_imeis: imeisBefore.length,
    imeis: imeisBefore.slice(0, 10),
  });
}

function hasLegacySubscribers() {
  const channelListeners = rooms.get(COMMAND_RESPONSE_CHANNEL)?.size || 0;
  return deviceSubscribers.size > 0 || channelListeners > 0;
}

function hasDeviceSubscribers(imei) {
  const key = imei ? String(imei).trim() : "";
  if (!key) return false;
  const clients = deviceSubscribers.get(key);
  return !!(clients && clients.size > 0);
}

function pruneDeadSubscribersForImei(imei) {
  const clients = deviceSubscribers.get(imei);
  if (!clients) return;
  for (const ws of clients) {
    if (!ws || ws.readyState !== WebSocket.OPEN) clients.delete(ws);
  }
  if (clients.size === 0) deviceSubscribers.delete(imei);
}

function pruneAllDeadSubscribers() {
  for (const imei of [...deviceSubscribers.keys()]) {
    pruneDeadSubscribersForImei(imei);
  }
}

function clearReconnectTimer() {
  if (typeof traccarReconnect !== "undefined" && traccarReconnect.clearTimer) {
    traccarReconnect.clearTimer();
  }
}

function shouldMaintainTraccarIngress() {
  return !RELAY_ONLY_LEGACY || hasLegacySubscribers();
}

async function ensureTraccarIngress(reason = "subscribers_joined") {
  if (!shouldMaintainTraccarIngress()) return;
  if (
    traccarWs &&
    (traccarWs.readyState === WebSocket.OPEN || traccarWs.readyState === WebSocket.CONNECTING)
  ) {
    return;
  }
  if (traccarIngressStarting || (traccarReconnect && traccarReconnect.inProgress)) return;

  traccarIngressStarting = true;
  try {
    if (!traccarHttpClient || !traccarCookieHeader) {
      const { cookieHeader, client } = await loginAndGetCookieHeader();
      traccarHttpClient = client;
      traccarCookieHeader = cookieHeader;
    }
    if (!devicesPollingStarted) startDevicesPolling();
    await fetchTraccarDevicesList(`ingress_${reason}`);
    traccarWs = startWebSocket(traccarCookieHeader);
    console.warn("[legacy-relay] Traccar ingress started", {
      reason,
      subscribed_imeis: deviceSubscribers.size,
      sockets: countDeviceSubscriberSockets(),
    });
  } catch (err) {
    console.warn("[legacy-relay] Traccar ingress start failed", {
      reason,
      error: err.message,
    });
    traccarReconnect.schedule("ingress_start_failed");
  } finally {
    traccarIngressStarting = false;
  }
}

function stopTraccarIngress(reason = "no_subscribers") {
  if (!RELAY_ONLY_LEGACY) return;

  clearReconnectTimer();
  closingForSubscriberIdle = true;
  try {
    if (traccarWs) {
      try {
        traccarWs.close();
      } catch (err) {
        console.warn("[legacy-relay] Traccar WS close error", { error: err.message });
      }
      traccarWs = null;
    }
  } finally {
    closingForSubscriberIdle = false;
  }

  if (devicesPollTimer) {
    clearInterval(devicesPollTimer);
    devicesPollTimer = null;
    devicesPollingStarted = false;
  }

  console.warn("[legacy-relay] Traccar ingress stopped", { reason });
}

function syncSubscriberIngressState(trigger) {
  if (!RELAY_ONLY_LEGACY) return;
  pruneAllDeadSubscribers();
  if (hasLegacySubscribers()) {
    void ensureTraccarIngress(trigger);
  } else {
    stopTraccarIngress(trigger);
  }
}
...
const wsDelivery = createWsDelivery({
  ...
  onTerminate: (ws) => {
    cleanupSubscriberSocket(ws, "backpressure");
    try { ws.terminate(); } catch { /* ignore */ }
  },
});

const traccarReconnect = createTraccarReconnect({
  baseMs: BRIDGE_ENV.TRACCAR_RECONNECT_BASE_MS,
  maxMs: BRIDGE_ENV.TRACCAR_RECONNECT_MAX_MS,
  metrics: bridgeMetrics,
  shouldConnect: () => !shuttingDown && shouldMaintainTraccarIngress(),
  isSocketHealthy: () =>
    !!(traccarWs && (traccarWs.readyState === WebSocket.OPEN || traccarWs.readyState === WebSocket.CONNECTING)),
  connect: async () => {
    if (traccarWs && traccarWs.readyState === WebSocket.OPEN) {
      closingForReconnect = true;
      try { traccarWs.close(); } catch (err) {
        console.warn("Failed to close existing Traccar WS", { error: err.message });
      }
    }
    const { cookieHeader, client } = await loginAndGetCookieHeader();
    traccarHttpClient = client;
    traccarCookieHeader = cookieHeader;
    await fetchTraccarDevicesList("ws_reconnect");
    traccarWs = startWebSocket(cookieHeader);
    traccarReconnect.notifySuccess();
  },
});
createTraccarSilenceWatchdog({
  silenceMs: BRIDGE_ENV.TRACCAR_SILENCE_WATCHDOG_MS,
  metrics: bridgeMetrics,
  isConnected: () => !!(traccarWs && traccarWs.readyState === WebSocket.OPEN),
  reconnect: (reason) => traccarReconnect.schedule(reason, { immediate: true }),
});

function scheduleReconnect(reason = "unknown", delayMs) {
  if (RELAY_ONLY_LEGACY && !hasLegacySubscribers()) return;
  if (delayMs === 0) return traccarReconnect.schedule(reason, { immediate: true });
  return traccarReconnect.schedule(reason);
}

async function reconnectTraccarSocket(reason = "manual") {
  if (RELAY_ONLY_LEGACY && !hasLegacySubscribers()) return;
  return traccarReconnect.attemptConnect(reason);
}
...
function cleanupSubscriberSocket(ws) {
  if (!ws) return;
  wsDelivery.clearSocketState(ws);
  removeClientFromAll(ws);
  leaveAllRooms(ws);
  if (RELAY_ONLY_LEGACY) detachLegacyWssClient(ws, "cleanup");
}
...
function subscribeClient(ws, imei) {
  ...
  if (!key) return;
  if (!deviceSubscribers.has(key)) {
    deviceSubscribers.set(key, new Set());
  }
  deviceSubscribers.get(key).add(ws);
  if (!ws.subscriptions) ws.subscriptions = new Set();
  ws.subscriptions.add(key);
  logSubscriberMonitor("اشتراك جهاز", {
    client_id: ws._legacyClientId || null,
    imei: key,
    listeners_on_imei: deviceSubscribers.get(key)?.size ?? 0,
    socket_imei_count: ws.subscriptions.size,
  });
  syncSubscriberIngressState("subscribe");
}
...
function unsubscribeClient(ws, imei) {
  ...
  clients.delete(ws);
  if (clients.size === 0) {
    deviceSubscribers.delete(key);
  }
  if (ws.subscriptions) ws.subscriptions.delete(key);
  logSubscriberMonitor("إلغاء اشتراك جهاز", {
    client_id: ws._legacyClientId || null,
    imei: key,
    listeners_on_imei: deviceSubscribers.get(key)?.size ?? 0,
    socket_imei_count: ws.subscriptions?.size ?? 0,
  });
  syncSubscriberIngressState("unsubscribe");
}
...
function removeClientFromAll(ws) {
  ...
  syncSubscriberIngressState("ws_close_internal");
}
...
function broadcastToDeviceSubscribers(imei, payload) {
  const key = imei ? String(imei).trim() : "";
  if (!key) return;
  if (isCommandResponsePayload(payload)) emitCommandResponseChannel(key, payload);
  const clients = deviceSubscribers.get(key);
  if (!clients || clients.size === 0) return;
  ...
  if (clients.size === 0) deviceSubscribers.delete(key);
}
...
function broadcastToDeviceSubscribersImmediate(imei, payload) {
  ...
  try {
    ...
    console.error("Subscriber JSON error:", e.message);
    return;
  }
  for (const ws of clients) {
    if (!ws || ws.readyState !== WebSocket.OPEN) {
    ...
    try {
      ...
      console.error("Subscriber send error:", e.message);
    }
  }
  if (clients.size === 0) deviceSubscribers.delete(key);
}
...
function startSubscribersServer() {
  ...
  app.get("/health", (req, res) => {
    bridgeMetrics.subscriber_count = countDeviceSubscriberSockets();
    bridgeMetrics.traccar_ws_connected = !!(traccarWs && traccarWs.readyState === WebSocket.OPEN);
    bridgeMetrics.gpslogs_write_enabled = GPSLOGS_WRITE_ENABLED;
    res.json({
      ok: true,
      service: RELAY_ONLY_LEGACY ? "traccar-bridge-legacy-relay" : "traccar-bridge",
      relay_only: RELAY_ONLY_LEGACY,
      gpslogs_write_enabled: GPSLOGS_WRITE_ENABLED,
      port: SUBSCRIBERS_SERVER_PORT,
      subscribers: getSubscriberMonitorSnapshot(),
      command_response_channel: COMMAND_RESPONSE_CHANNEL,
      command_response_channel_listeners:
        rooms.get(COMMAND_RESPONSE_CHANNEL)?.size || 0,
      traccar_ws_state: traccarWs ? traccarWs.readyState : null,
      reconnect: traccarReconnect.snapshot(),
      ts: new Date().toISOString(),
      metrics: snapshotMetrics(bridgeMetrics, {
        max_live_fix_age_ms: MAX_LIVE_FIX_AGE_MS,
        ws_refresh_ms: WS_REFRESH_MS,
   ...
   * Works even on legacy relay (Mongo connects via mongo.js require).
   * Example: https://alfursantracking.com:2053/notify-test?user_id=42
   * Production preferred: http://127.0.0.1:3053/notify-test?user_id=42 (ontherport)
   */
    ...
    try {
      ...
      if (!mongoose || mongoose.connection.readyState !== 1) {
        return res.status(503).json({
          ...
          message: "MongoDB not connected yet; retry shortly or use ontherport :3053",
        });
      }
  ...
  app.get("/snot", async (req, res) => {
    return res.status(503).json({
      ok: false,
      error: "disabled_on_legacy_relay",
      message: "FCM test disabled on legacy port; use traccar-bridge-ontherport.js",
    });
  });
  ...
  app.post("/traccar/reconnect", (req, res) => {
    return res.status(503).json({
      ok: false,
      error: "disabled_on_legacy_relay",
      message: "Use traccar-bridge-ontherport.js /traccar/reconnect for persistence bridge",
    });
  });

  /**
   * Laravel / داخلي: ضبط المسافة المجمّعة وساعات المحرك في Traccar.
   * Disabled on legacy relay; persistence process owns this endpoint.
   */
  app.post("/traccar/accumulators", async (req, res) => {
    return res.status(503).json({
      ok: false,
      error: "disabled_on_legacy_relay",
      message: "Use traccar-bridge-ontherport.js /traccar/accumulators",
    });
  });
  ...
  app.post("/send", async (req, res) => {
    try {
      const body = req.body || {};
      const { imei, command, deviceId } = body;
      ...
      if (!traccarHttpClient) {
        return res.status(503).send("❌ Traccar session is not ready");
      }
      ...
      if (!Number.isFinite(targetDeviceId)) {
        return res.status(404).send("❌ device mapping not found");
      }
      ...
      return res.send("✅ تم إرسال الأمر للجهاز عبر Traccar (legacy relay — بدون تخزين Mongo/FCM)");
    } catch (err) {
  ...
  wss.on("connection", (ws) => {
    ...
    if (RELAY_ONLY_LEGACY) attachLegacyWssClient(ws);

    sendWsSafe(ws, {
      type: "welcome",
      mode: RELAY_ONLY_LEGACY ? "traccar-bridge-legacy-relay" : "traccar-bridge",
      relay_only: RELAY_ONLY_LEGACY,
    ...
    ws.on("message", async (msg) => {
      ...
      if (
        ...
        if (RELAY_ONLY_LEGACY) syncSubscriberIngressState("subscribe_cmd_channel");
        sendWsSafe(ws, {
      ...
      if (
        ...
        if (RELAY_ONLY_LEGACY) syncSubscriberIngressState("unsubscribe_cmd_channel");
        sendWsSafe(ws, {
  ...
  server.listen(SUBSCRIBERS_SERVER_PORT, "0.0.0.0", () => {
    console.log(`Bridge HTTPS+WS listening on https://0.0.0.0:${SUBSCRIBERS_SERVER_PORT}`);
  });
}
...
async function upsertDeviceMap(mysqlId, imei) {
  const id = Number(mysqlId);
  const normImei = String(imei || "").trim();
  if (Number.isFinite(id) && normImei) deviceIdToImeiCache.set(id, normImei);
}
...
async function resolveDeviceIdByImei(imei) {
  ...
  for (const [id, cached] of deviceIdToImeiCache.entries()) {
    if (String(cached) === normalized) return id;
  }

  if (traccarHttpClient) {
    try {
      const { data } = await traccarHttpClient.get("/api/devices", { params: { uniqueId: normalized } });
      const list = Array.isArray(data) ? data : data ? [data] : [];
      const match = list.find((d) => String(d?.uniqueId || "").trim() === normalized);
      if (match?.id != null) {
        const id = Number(match.id);
        deviceIdToImeiCache.set(id, normalized);
        return id;
      }
    } catch (err) {
      console.warn("resolveDeviceIdByImei Traccar lookup failed", err.message);
    }
  }
  return null;
}
...
/**
 * Command response: broadcast immediately. Persistence lives on ontherport.
 */
function persistCommandResponseFast(imei, position, commandResponseText) {
  const key = imei ? String(imei).trim() : "";
  if (!key) return;
  const hasChannelListeners = (rooms.get(COMMAND_RESPONSE_CHANNEL)?.size || 0) > 0;
  if (RELAY_ONLY_LEGACY && !hasDeviceSubscribers(key) && !hasChannelListeners) return;

  const wirePayload = buildCommandResponseSubscriberPayload(key, position, commandResponseText);
  cmdChannelBp("persistCommandResponseFast", {
    imei: key,
    responsePreview: String(commandResponseText || "").slice(0, 80),
  });
  broadcastToDeviceSubscribersImmediate(key, wirePayload);
}
...
function persistPosition(position, rawPayload, options = {}) {
  if (RELAY_ONLY_LEGACY && !hasLegacySubscribers()) return Promise.resolve();

  const imei = resolveImeiFromCache(position?.deviceId);
  if (!imei) {
    void resolveImei(position?.deviceId)
      .then((resolved) => {
        if (!resolved) {
          console.warn("Skipping position without IMEI mapping for deviceId:", position?.deviceId);
          return;
        }
        return persistPositionBody(resolved, position, rawPayload, options);
      })
      .catch((err) => console.error("resolveImei error:", err.message));
    return Promise.resolve();
  }
  return persistPositionBody(imei, position, rawPayload, options);
}
...
function processGpsBurst(imei, positions, rawPayload) {
  ...
  const picked = pickLatestLiveFromBurst(list, {
    lastLiveFixMs: liveFixTracker.peek(imei),
  ...
  for (let i = 0; i < list.length; i++) {
    persistPositionBody(imei, list[i], rawPayload, {
      emitLive: i === picked.liveIndex,
      burstSuperseded: picked.liveIndex >= 0 && i !== picked.liveIndex,
    });
  }
}

async function persistPositionBody(imei, position, rawPayload, options = {}) {
  const imeiKey = imei ? String(imei).trim() : "";
  if (!imeiKey) return;

  const attrs = position?.attributes || {};
  if (positionHasCommandResponse(position)) {
    // Channel-only listeners are enough (no per-IMEI subscribe required)
    persistCommandResponseFast(imeiKey, position, String(attrs.result).trim());
    return;
  }

  if (RELAY_ONLY_LEGACY && !hasDeviceSubscribers(imeiKey)) return;
  ...
  if (emitLive && cls.liveEligible && liveFixTracker.acceptLive(imei, cls.effectiveLiveMs, {
    source: cls.effectiveLiveSource,
    fixMs: cls.fixMs,
  })) {
    emitPositionToSocketSubscribers(imei, doc.type, subscriberData, { immediate: true });
    recordLiveDecision(bridgeMetrics, "fresh");
  } else {
...
async function persistEvent(eventObj) {
  ...
  if (!imei) return;
  if (RELAY_ONLY_LEGACY && !hasDeviceSubscribers(imei)) return;
...
async function persistTraccarDeviceStatus(traccarDevice) {
  ...
  const status = traccarDevice?.status ?? null;
  ...
  const legacyDoc = {
    ...
    status,
    lastUpdate,
    truccer_dev_status: traccarDevice,
  };
...
function startWebSocket(cookieHeader) {
  const ws = new WebSocket(TRACCAR_WS, { headers: { Cookie: cookieHeader } });
  ...
  ws.on("close", (code, reason) => {
    ...
    if (closingForReconnect || closingForSubscriberIdle) return;
...
async function bootBridge() {
  ...
  if (RELAY_ONLY_LEGACY) {
    console.log(
      `[legacy-relay] Traccar HTTP session ready — port ${SUBSCRIBERS_SERVER_PORT}; Traccar WS starts when first subscriber connects`
    );
    logSubscriberMonitor("السيرفر جاهز", { port: SUBSCRIBERS_SERVER_PORT });
    if (WS_REFRESH_MS > 0) {
      setInterval(() => {
        if (hasLegacySubscribers()) scheduleReconnect("periodic_refresh", 0);
      }, WS_REFRESH_MS);
    }
    return;
  }

  console.log("Logged in to Traccar.");
  startDevicesPolling();
