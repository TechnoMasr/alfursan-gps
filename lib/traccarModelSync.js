...
function createTraccarModelSync({
  ...
  function canUseSyncedState(imei, runtimeDeviceId, currentModel) {
    const cached = getCachedModel(imei);
    ...
    if (currentModel && normalizeModel(currentModel) !== cached.trModel) return false;
    return true;
  }
  ...
  function schedule(task) {
    ...
    if (canUseSyncedState(imei, runtimeDeviceId, currentModel)) {
  ...
  async function runTask(key, task) {
    try {
      ...
      if (!client || typeof client.put !== "function") throw new Error("traccar_client_not_ready");
      ...
      await client.put(`/api/devices/${task.runtimeDeviceId}`, updatedDevice);
      syncedRuntime.set(runtimeKey(task.imei, task.runtimeDeviceId), {
        trModel: desiredModel,
        syncedAt: now(),
      });
