...
function loadBridgeEnv(overrides = {}) {
  const env = {
    ...
    MILEAGE_CHUNK_SIZE: intEnv("MILEAGE_CHUNK_SIZE", 250),
    MILEAGE_BUSINESS_DAY: strEnv("MILEAGE_BUSINESS_DAY", "cairo"),
    MILEAGE_OVERSPEED_SOURCE: strEnv("MILEAGE_OVERSPEED_SOURCE", "alerts"),
    TRAVEL_CHUNK_SIZE: intEnv("TRAVEL_CHUNK_SIZE", 150),
    TRAVEL_BUSINESS_DAY: strEnv("TRAVEL_BUSINESS_DAY", "cairo"),
    IDLE_CHUNK_SIZE: intEnv("IDLE_CHUNK_SIZE", 150),
    IDLE_BUSINESS_DAY: strEnv("IDLE_BUSINESS_DAY", "cairo"),
    STATIC_CHUNK_SIZE: intEnv("STATIC_CHUNK_SIZE", 500),
    STATIC_FALLBACK_CHUNK_SIZE: intEnv("STATIC_FALLBACK_CHUNK_SIZE", 100),
    ...
    // Negative cache TTL retained for env compatibility; blank Mongo now defaults to Seeworld.
    ...
    /** Periodic Traccar GET /api/devices?all=true registry refresh (memory-mode). */
    DEVICES_POLL_MS: intEnv("DEVICES_POLL_MS", 900_000),
    /** After Traccar restart, all=true may briefly return []. Defer mass-offline during grace. */
    TRACCAR_REGISTRY_STARTUP_GRACE_MS: intEnv("TRACCAR_REGISTRY_STARTUP_GRACE_MS", 120_000),
    /** Retry interval while empty snapshot is deferred / awaiting confirmation. */
    TRACCAR_REGISTRY_RECONCILE_RETRY_MS: intEnv("TRACCAR_REGISTRY_RECONCILE_RETRY_MS", 15_000),
