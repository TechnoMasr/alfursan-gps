...
const { partitionPositions, groupByDeviceId } = require("./ingressPartition");
...
function createPositionPipeline(options = {}) {
  ...
  /**
   * Process one Traccar socket payload. Commands first, then GPS burst coalesce.
   * Persistence is queued, never awaited.
   */
  function handleTraccarPayload(data, hooks = {}) {
    const receivedAt = now();
    metrics.last_traccar_message_at = new Date(receivedAt).toISOString();
    if (!data || typeof data !== "object") return { receivedAt };
    ...
    if (Array.isArray(data.positions) && data.positions.length) {
      ...
      metrics.traccar_positions_received_total =
        (metrics.traccar_positions_received_total || 0) + data.positions.length;
      const { commands, alarms, gps } = partitionPositions(data.positions);
      const rest = [...alarms, ...gps];

      for (const position of commands) {
        const imei = resolveImeiSync?.(position.deviceId);
        if (imei) {
          hooks.onCommand?.(imei, position);
        } else {
          hooks.onCommandUnresolved?.(position);
        }
      }

      const grouped = groupByDeviceId(rest);
      for (const [deviceId, list] of grouped.entries()) {
        const imei = resolveImeiSync?.(deviceId);
        if (imei) {
          hooks.onGpsBurst?.(imei, list, receivedAt);
        } else if (typeof resolveImeiAsync === "function") {
          const pending = list.slice();
          void resolveImeiAsync(deviceId).then((resolved) => {
            if (resolved) hooks.onGpsBurst?.(resolved, pending, receivedAt);
            else onUnresolvedBatch?.(deviceId, pending);
          });
        } else {
          onUnresolvedBatch?.(deviceId, list);
        }
      }
    }
