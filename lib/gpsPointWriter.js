...
function createGpsPointWriter(options = {}) {
  const {
    ...
    /** E1: max legacy files opened per flush cycle (combine into Mongo batches). */
    maxFilesPerCycle = 500,
    /** Fraction of doc budget reserved for oldest spool files (rest → newest). */
  ...
  function applyMetaToMetrics() {
    ...
    // Doc count when known; else file count (legacy meta without docs estimate).
  ...
  /**
   * E1: pick files with DOC-based old/new fairness (not 2-new/1-old file slots).
   * Returns file paths alternating toward oldest and newest until doc/file budgets hit.
   */
  function pickFairFilesByDocs(files) {
    ...
    // Legacy knobs: at least open this many files from each side when present.
    ...
    function take(path, side) {
      if (!path || seen.has(path)) return false;
      seen.add(path);
      selected.push({ path, side });
      if (side === "old") oldFiles += 1;
      else newFiles += 1;
      return true;
    }

    // Estimate docs from meta when available (1 if unknown — matches tiny legacy files).
    ...
    while (selected.length < maxFiles && iOld <= iNew) {
      const needOld =
        oldDocs < oldBudget || oldFiles < minOldFiles;
      const needNew =
        newDocs < newBudget || newFiles < minNewFiles;
      ...
      let side;
      if (needOld && needNew) {
        // Prefer the side farther behind its budget (doc-based).
        side = oldDocs <= newDocs ? "old" : "new";
      ...
      if (side === "old") {
        ...
        if (iOld > iNew) break;
        const path = files[iOld++];
        if (take(path, "old")) oldDocs += estDocs(path);
      } else {
        while (iNew >= iOld && seen.has(files[iNew])) iNew -= 1;
        if (iNew < iOld) break;
        const path = files[iNew--];
        if (take(path, "new")) newDocs += estDocs(path);
      }
    }
    return selected;
  }
  ...
  /**
   * E1: open many legacy files, COMBINE docs into insertMany batches up to batchSize.
   * One Mongo op per batch — not per file.
   */
  async function drainSpoolFair() {
    ...
    const fileStates = [];
    for (const { path: filePath } of picked) {
      let docs;
      try {
        docs = parseSpoolFile(filePath);
      } catch {
      ...
      // Refresh meta docs count for better future fairness estimates.
      const meta = spoolMeta.get(filePath);
    ...
    while (mongoBatchesThisCycle < maxMongoBatchesPerCycle) {
      ...
      try {
        ...
        // Leave pending as-is; files unchanged on disk for retry.
        throw err;
      }
      ...
      for (const ref of refs) {
        ...
        if (acked.has(doc)) ref.state.status[ref.index] = "acked";
          ...
          // invalid / unexpected duplicate already quarantined — treat as done
          ref.state.status[ref.index] = "acked";
        }
      }

      if (classified.retry.length && !classified.acked.length) {
        // Pure retryable failure already counted; stop cycle for backoff.
        break;
      }
    }
    ...
    for (const state of fileStates) {
      ...
      // else: untouched file (not reached in this cycle) — leave on disk
    }
    applyMetaToMetrics();
  }
