...
function createGpsPointWriter(options = {}) {
  const {
    ...
    /** E2: time-based seal (ms). Prefer this; ACK does not require seal. Default 5s. */
    segmentMaxAgeMs,
    /** @deprecated alias for segmentMaxAgeMs */
    segmentSealMs,
    /**
  ...
  const sealAgeMs = (() => {
    if (segmentMaxAgeMs != null && segmentMaxAgeMs !== "") {
      const n = Number(segmentMaxAgeMs);
      return Number.isFinite(n) ? Math.max(0, n) : 5000;
    }
    if (segmentSealMs != null && segmentSealMs !== "") {
      const n = Number(segmentSealMs);
      return Number.isFinite(n) ? Math.max(0, n) : 5000;
    }
    return 5000;
  })();
  ...
  function scheduleSealDeadline() {
    if (!enableJournal || sealTimer || !sealAgeMs) return;
    if (!activePath || activeDocs <= 0) return;
    const age = now() - activeOpenedAt;
    const delay = Math.max(0, sealAgeMs - age);
  ...
  function shouldSealActive() {
    ...
    if (sealAgeMs > 0 && now() - activeOpenedAt >= sealAgeMs) return true;
    return false;
  }
  ...
  function writeHeartbeat() {
    ...
    try {
      ensureDir(resolvedSpoolDir);
      applyMetaToMetrics();
      const hbTs = new Date(now()).toISOString();
      const body = JSON.stringify({
        alive: true,
        pid: drainOwnerId,
        mode: resolvedMode,
        ts: hbTs,
        // External writer Mongo counters (not producer-local).
        persisted_total: metrics.gpspoints_persisted_total || 0,
        mongo_attempted_total: metrics.gpspoints_mongo_attempted_total || 0,
        mongo_acknowledged_total: metrics.gpspoints_mongo_acknowledged_total || 0,
        mongo_flush_count: metrics.gpspoints_mongo_flush_count || 0,
        batch_docs_last: metrics.gpspoints_mongo_docs_per_flush_last || 0,
        batch_docs_avg: metrics.gpspoints_mongo_docs_per_flush_avg || 0,
        batch_docs_max: metrics.gpspoints_mongo_docs_per_flush_max || 0,
        mongo_latency_ms: metrics.gpspoints_mongo_latency_ms || 0,
        mongo_latency_p50_ms: metrics.gpspoints_mongo_latency_p50 || 0,
        mongo_latency_p95_ms: metrics.gpspoints_mongo_latency_p95 || 0,
        mongo_latency_p99_ms: metrics.gpspoints_mongo_latency_p99 || 0,
        retry_total: metrics.gpspoints_retry_total || metrics.gpspoints_writer_retries || 0,
        persist_failures: metrics.gpspoints_persist_failures || 0,
        quarantined_total:
          metrics.gpspoints_quarantined_total || metrics.gpspoints_writer_quarantined || 0,
        ...
        backlog_bytes: metrics.gpspoints_backlog_bytes || 0,
        backlog_oldest_age_ms: metrics.gpspoints_backlog_oldest_age || 0,
      });
      ...
      metrics.gpspoints_writer_last_heartbeat = hbTs;
    } catch (err) {
...
/**
 * Read dedicated gpspoints-writer status snapshot (atomic heartbeat file).
 * Never throws — missing/corrupt file returns safe zeros / alive=false.
 * These are EXTERNAL WRITER metrics, not producer-local gpspoints_* counters.
 */
function readGpspointsWriterHeartbeat(spoolDir, fsImpl = fs, clock = Date.now) {
  ...
  const empty = {
    ...
    gpspoints_writer_persisted_total: 0,
    gpspoints_writer_mongo_attempted_total: 0,
    gpspoints_writer_mongo_acknowledged_total: 0,
    gpspoints_writer_mongo_flush_count: 0,
    gpspoints_writer_batch_docs_last: 0,
    gpspoints_writer_batch_docs_avg: 0,
    gpspoints_writer_batch_docs_max: 0,
    gpspoints_writer_mongo_latency_ms: 0,
    gpspoints_writer_mongo_latency_p50_ms: 0,
    gpspoints_writer_mongo_latency_p95_ms: 0,
    gpspoints_writer_mongo_latency_p99_ms: 0,
    gpspoints_writer_retry_total: 0,
    gpspoints_writer_persist_failures: 0,
    gpspoints_writer_quarantined_total: 0,
    gpspoints_writer_backlog_docs: 0,
    gpspoints_writer_backlog_files: 0,
    gpspoints_writer_backlog_bytes: 0,
    gpspoints_writer_backlog_oldest_age_ms: 0,
  };
  try {
    const raw = fsImpl.readFileSync(heartbeatPath, "utf8");
    const data = JSON.parse(raw);
    if (!data || typeof data !== "object") return empty;
    const ts = Date.parse(data.ts || data.last_heartbeat || "");
    ...
    return {
      ...
      gpspoints_writer_persisted_total: numOr(data.persisted_total),
      gpspoints_writer_mongo_attempted_total: numOr(data.mongo_attempted_total),
      gpspoints_writer_mongo_acknowledged_total: numOr(data.mongo_acknowledged_total),
      gpspoints_writer_mongo_flush_count: numOr(data.mongo_flush_count),
      gpspoints_writer_batch_docs_last: numOr(data.batch_docs_last),
      gpspoints_writer_batch_docs_avg: numOr(data.batch_docs_avg),
      gpspoints_writer_batch_docs_max: numOr(data.batch_docs_max),
      gpspoints_writer_mongo_latency_ms: numOr(data.mongo_latency_ms),
      gpspoints_writer_mongo_latency_p50_ms: numOr(data.mongo_latency_p50_ms),
      gpspoints_writer_mongo_latency_p95_ms: numOr(data.mongo_latency_p95_ms),
      gpspoints_writer_mongo_latency_p99_ms: numOr(data.mongo_latency_p99_ms),
      gpspoints_writer_retry_total: numOr(data.retry_total),
      gpspoints_writer_persist_failures: numOr(data.persist_failures),
      gpspoints_writer_quarantined_total: numOr(data.quarantined_total),
      gpspoints_writer_backlog_docs: numOr(data.backlog_docs),
      gpspoints_writer_backlog_files: numOr(data.backlog_files),
      gpspoints_writer_backlog_bytes: numOr(data.backlog_bytes),
      gpspoints_writer_backlog_oldest_age_ms: numOr(data.backlog_oldest_age_ms),
    };
  } catch {
    return empty;
  }
