...
const INGRESS_MODES = new Set(["ws", "forward-shadow", "forward"]);

function normalizeIngressMode(value) {
  const mode = String(value || "ws").trim().toLowerCase();
  return INGRESS_MODES.has(mode) ? mode : "ws";
}
...
function validateForwardItem(item) {
  const deviceIdNum = Number(item.deviceId);
...
function normalizeForwardPayload(body) {
  ...
  for (const rawPosition of list) {
    ...
    if (reason) invalid.push({ reason, deviceId: item.deviceId ?? null, imei: item.imei ?? null });
...
function positionShadowSignature(position) {
  ...
  return [
    ...
    Number(position.speed || 0).toFixed(3),
    attrs.alarm || "",
    attrs.result || "",
  ].join("|");
}
