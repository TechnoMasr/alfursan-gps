...
function startGlobalReportSchedulers(options = {}) {
  ...
  async function runMileage() {
    return runtime.gates.mileage.run(async () => {
      await updateIncrementalMileage({ metrics });
      await buildAndPersistDailyReport(undefined, { metrics });
    });
  }
  ...
  async function runStatic() {
    return runtime.gates.static.run(async () => {
      ...
      await buildAndPersistStaticStats({ mileageThresholdKm, metrics });
    });
  }
